...
}; // struct UnlogDefaultRuntimeSettings
...
#define UN_LOG( InMacroArgs, VerbosityName, Message) \
    ...
    ( ELogVerbosity::VerbosityName, FUNCTION_TEXT(Message));
...
#define UN_MESSAGE_LOG(InLogCategory, InVerbosity, InMessage) \
    Target::MessageLog::Call(InLogCategory, ELogVerbosity::InVerbosity, InMessage);

#define UN_MESSAGE_LOGF(InLogCategory, InVerbosity, InMessage, ...) \
    Target::MessageLog::Call(InLogCategory, ELogVerbosity::InVerbosity, FString::Printf(TEXT(InMessage), ##__VA_ARGS__));

#define UN_CLOG( Condition, InMacroArgs, VerbosityName, Message) \
    { \
        if( Condition ) \
        {\
            UN_LOG( InMacroArgs, VerbosityName, Message); \
        }\
    }
...
#define UN_CMESSAGE_LOG( Condition, InLogCategory, InVerbosity, InMessage) \
    {\
        if( Condition ) \
        {\
            UN_MESSAGE_LOG( InLogCategory, InVerbosity, InMessage); \
        }\
    }
#define UN_CMESSAGE_LOGF( Condition, InLogCategory, InVerbosity, InMessage, ... ) \
    {\
        if( Condition ) \
        {\
            UN_MESSAGE_LOGF( InLogCategory, InVerbosity, InMessage, ##__VA_ARGS__); \
        }\
    }
...
#define UN_MESSAGE_LOG(...) UNLOG_COMPILED_OUT
#define UN_MESSAGE_LOGF(...) UNLOG_COMPILED_OUT
...
using Unlog = TUnlog<Target::UELog>;
